...
class BowlerStats:
    ...
    def player_match_stats(self, players):
        ...
        for player in players:
            ...
            row = [self.overs_faced(match_deliveries), self.runs_conceded(match_deliveries), self.wickets(match_deliveries), self.best_bowling_figures(match_deliveries), self.economy(match_deliveries), self.city(best_match), self.season(best_match)]
            stats.append(row)

        columns = ['Ov', 'Runs', 'Wkts', 'BBF', 'Econ', 'City', 'Season']
        return print (pd.DataFrame(stats, index=[players], columns=columns).sort_values(by='Wkts', ascending=False) )


# Player Summary Statistics 
players = ['JJ Bumrah', 'Rashid Khan', 'JR Hazlewood', 'K Rabada', 'PJ Cummins', 'YS Chahal', 'AD Russell']
